...
const handelDataFetchCheckout = async (path, method, body) => { // callback function for animation
    ...
    try {
        ...
        if (data.code === 'TOKEN_EXPIRED' || res.status === 401) {
            ...
            try {
                const newTokens = await handelRefreshToken(); // Assume this now only handles localStorage
                ...
                const resRetry = await handelRetryRequest(path, method, body, newTokens.accessToken);
